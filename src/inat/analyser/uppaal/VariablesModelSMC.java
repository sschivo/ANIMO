		...
		try {
			...
			for (Reaction r : m.getReactionCollection()) {
				...
				if (r.get(CATALYST).as(String.class).equals(r.get(REACTANT).as(String.class)) && r.get(SCENARIO).as(Integer.class) != 0) { //R1 == R2: use only r1_reacting to avoid input nondeterminism
					...
					document = documentBuilder.parse(new ByteArrayInputStream(("<template><name x=\"5\" y=\"5\">Reaction_" + r.get(CATALYST).as(String.class) + "_" + r.get(REACTANT).as(String.class) + ((r.get(OUTPUT_REACTANT).as(String.class).equals(r.get(REACTANT).as(String.class))) ? "" : "_" + r.get(OUTPUT_REACTANT).as(String.class)) + "</name><parameter>int &amp;reactant1, int &amp;reactant2, int &amp;output, const int delta, broadcast chan &amp;r1_reacting, broadcast chan &amp;r2_reacting, broadcast chan &amp;output_reacting</parameter><declaration>clock c;\nint r1, r2;\ntime_t T, tL, tU;\nconst double k = " + formatDouble(r.getProperties().get(Model.Properties.SCENARIO_PARAMETER_K).as(Double.class)) + ";\nconst double levelsScale = " + formatDouble(r.get(Model.Properties.LEVELS_SCALE_FACTOR + "_reaction").as(Double.class)) + ";\nconst double scale = multiply(levelsScale, timeScale);\nconst double R1Levels = " + formatDouble(1.0 * m.getReactant(r.get(CATALYST).as(String.class)).get(NUMBER_OF_LEVELS).as(Integer.class)) + ";\ndouble r;\n\nvoid update() {\n\tr = scenario1(k, int_to_double(reactant1), R1Levels, true);\n\tif (r.b != 0) {\n\t\tdouble time = multiply(inverse(r), scale);\n\t\ttL = round(multiply(time, uncertaintyLower));\n\t\ttU = round(multiply(time, uncertaintyUpper));\n\t} else {\n\t\ttL = INFINITE_TIME;\n\t\ttU = INFINITE_TIME;\n\t}\n\tif (tL != INFINITE_TIME &amp;&amp; tU != INFINITE_TIME &amp;&amp; tL &gt; tU) { //We use rounded things: maybe the difference between tL and tU was not so great, and with some rounding problems we could have this case\n\t\ttL = tU;\n\t}\n}\n\nvoid react() {\n\toutput = output + delta;\n\tupdate();\n}\n</declaration><location id=\"id0\" x=\"-1512\" y=\"-696\"><name x=\"-1584\" y=\"-688\">stubborn</name><committed/></location><location id=\"id1\" x=\"-1384\" y=\"-976\"><name x=\"-1392\" y=\"-1008\">not_reacting</name></location><location id=\"id2\" x=\"-960\" y=\"-856\"><name x=\"-952\" y=\"-880\">resetting</name><committed/></location><location id=\"id3\" x=\"-1384\" y=\"-768\"><name x=\"-1416\" y=\"-808\">reacting</name><label kind=\"invariant\" x=\"-1624\" y=\"-792\">tU == INFINITE_TIME\n|| c&lt;=tU</label></location><location id=\"id4\" x=\"-1384\" y=\"-896\"><committed/></location><location id=\"id5\" x=\"-1248\" y=\"-624\"><name x=\"-1360\" y=\"-616\">about_to_react</name></location><location id=\"id6\" x=\"-1536\" y=\"-896\"><name x=\"-1568\" y=\"-928\">start</name><committed/></location><init ref=\"id6\"/><transition><source ref=\"id0\"/><target ref=\"id2\"/><label kind=\"guard\" x=\"-1624\" y=\"-656\">c&lt;tL</label><label kind=\"assignment\" x=\"-1624\" y=\"-640\">update()</label><nail x=\"-1512\" y=\"-592\"/><nail x=\"-928\" y=\"-592\"/><nail x=\"-928\" y=\"-776\"/></transition><transition><source ref=\"id0\"/><target ref=\"id3\"/><label kind=\"guard\" x=\"-1632\" y=\"-728\">c&gt;=tL</label><nail x=\"-1512\" y=\"-744\"/></transition><transition><source ref=\"id3\"/><target ref=\"id0\"/><label kind=\"synchronisation\" x=\"-1496\" y=\"-736\">r1_reacting?</label><nail x=\"-1408\" y=\"-720\"/><nail x=\"-1488\" y=\"-720\"/></transition><transition><source ref=\"id5\"/><target ref=\"id3\"/><label kind=\"synchronisation\" x=\"-1272\" y=\"-672\">output_reacting?</label><label kind=\"assignment\" x=\"-1272\" y=\"-656\">c := tU</label><nail x=\"-1144\" y=\"-624\"/><nail x=\"-1144\" y=\"-680\"/><nail x=\"-1344\" y=\"-680\"/></transition><transition><source ref=\"id1\"/><target ref=\"id2\"/><label kind=\"synchronisation\" x=\"-1640\" y=\"-1016\">r1_reacting?</label><label kind=\"assignment\" x=\"-1536\" y=\"-1016\">update(), c:=0</label><nail x=\"-1416\" y=\"-1000\"/><nail x=\"-1672\" y=\"-1000\"/><nail x=\"-1672\" y=\"-576\"/><nail x=\"-912\" y=\"-576\"/><nail x=\"-912\" y=\"-792\"/></transition><transition><source ref=\"id3\"/><target ref=\"id5\"/><label kind=\"guard\" x=\"-1448\" y=\"-672\">c&gt;=tL\n&amp;&amp; (output+delta&gt;" + m.getReactant(r.get(OUTPUT_REACTANT).as(String.class)).get(NUMBER_OF_LEVELS).as(Integer.class) + "\n|| output+delta&lt;0)</label><nail x=\"-1384\" y=\"-680\"/><nail x=\"-1456\" y=\"-680\"/><nail x=\"-1456\" y=\"-624\"/></transition><transition><source ref=\"id3\"/><target ref=\"id2\"/><label kind=\"guard\" x=\"-1128\" y=\"-736\">c&gt;=tL\n&amp;&amp; output+delta&gt;=0\n&amp;&amp; output+delta&lt;=" + m.getReactant(r.get(OUTPUT_REACTANT).as(String.class)).get(NUMBER_OF_LEVELS).as(Integer.class) + "</label><label kind=\"synchronisation\" x=\"-1128\" y=\"-696\">output_reacting!</label><label kind=\"assignment\" x=\"-1128\" y=\"-680\">react(),\nc:=0</label><nail x=\"-1320\" y=\"-696\"/><nail x=\"-936\" y=\"-696\"/><nail x=\"-936\" y=\"-768\"/></transition><transition><source ref=\"id2\"/><target ref=\"id1\"/><label kind=\"guard\" x=\"-1328\" y=\"-992\">tL == INFINITE_TIME</label><nail x=\"-960\" y=\"-976\"/></transition><transition><source ref=\"id4\"/><target ref=\"id1\"/><label kind=\"guard\" x=\"-1536\" y=\"-952\">tL == INFINITE_TIME</label></transition><transition><source ref=\"id2\"/><target ref=\"id3\"/><label kind=\"guard\" x=\"-1320\" y=\"-896\">tU != INFINITE_TIME\n&amp;&amp; c&gt;tU</label><label kind=\"assignment\" x=\"-1320\" y=\"-872\">c:=tU,\nr1:=reactant1,T:=tL</label><nail x=\"-1328\" y=\"-856\"/></transition><transition><source ref=\"id2\"/><target ref=\"id3\"/><label kind=\"guard\" x=\"-1320\" y=\"-824\">(tU == INFINITE_TIME\n&amp;&amp; tL != INFINITE_TIME)\n|| (tU != INFINITE_TIME\n&amp;&amp; c&lt;=tU)</label><label kind=\"assignment\" x=\"-1320\" y=\"-768\">r1:=reactant1,T:=tL</label><nail x=\"-960\" y=\"-752\"/><nail x=\"-1328\" y=\"-752\"/></transition><transition><source ref=\"id4\"/><target ref=\"id3\"/><label kind=\"guard\" x=\"-1584\" y=\"-880\">tL\n   != INFINITE_TIME</label><label kind=\"assignment\" x=\"-1504\" y=\"-848\">r1 := reactant1,\nT := tL,\nc := 0</label></transition><transition><source ref=\"id6\"/><target ref=\"id4\"/><label kind=\"assignment\" x=\"-1496\" y=\"-912\">update()</label></transition></template>").getBytes()));

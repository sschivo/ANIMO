...
public class VariablesModelReactantCentered extends VariablesModel {
	...
	protected void appendModel(StringBuilder out, Model m) {
		...
		out.append(newLine);
		out.append(newLine);
		out.append("typedef int[0, 200999] collapsedDouble; //A double number \"collapsed\" into a single integer: to collapse number d, the formula is ((d.b / 10) * 201) + (100 + d.e) where 201 = 2 * 100 + 1 and 100 is the maximum value of the .e in a double. 200999 = 999 * 201 + 200.");
		out.append(newLine);
		out.append("\t\t\t//Notice that we must sacrifice one significant figure from the .b in order to save the entire value of .e. This happens because the values &gt; 1e6 are output by UPPAAL with the exponential notation, losing at least one digit, and that digit is fundamental to recover the value of .e");
		out.append(newLine);
		out.append(newLine);
		...
		out.append(newLine);
		out.append("\tdouble_t r = {-1000, -100};");
		out.append(newLine);
		out.append("\tif (a.b == 0) {");
		out.append(newLine);
		out.append("\t\tr.b = -b.b;");
		...
		out.append(newLine);
		out.append(newLine);
		out.append("collapsedDouble collapse(double_t a) { //\"collapse\" a double number to a single integer. The .b MUST be in the [-9999, 9999] interval (and the .e must be in the [-100, 100] interval)");
		out.append(newLine);
	...
	protected void appendReactantVariables(StringBuilder out, Reactant r) {
		...
		if (!r.get(Model.Properties.LINEAR_SCALE).as(Boolean.class)) { //For log-scale, we use the "double" version of the variable
			out.append("double_t " + r.getId() + "_d := " + formatDouble(1.0 * r.get(Model.Properties.INITIAL_LEVEL).as(Integer.class)) + ";");
			out.append(newLine);
	...
	private String collapseDouble(double d) { //Convert the given number in its double_represented_as_a_single_integer ("collapsedDouble") representation
		Pair<Integer, Integer> dD = createDouble(d);
		return "" + (201 * dD.first/10 + (dD.second + 100));
	}

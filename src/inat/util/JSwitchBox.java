...
/**
 * Thanks to stackoverflow user oliholz http://stackoverflow.com/users/307767/oliholz
 * This class was copied from his answer here: http://stackoverflow.com/questions/7304017/how-to-add-sexy-on-off-sliders
 */
public class JSwitchBox extends AbstractButton{
    private static final long serialVersionUID = -3643872386353661230L;
    ...
    private Color red = new Color(255, 153, 153); //Color.RED.brighter().brighter(); //new Color(160,130,130);
    private Color green = new Color(153, 255, 153); //Color.GREEN.brighter().brighter(); //new Color(130,160,130);
    private Font font = new JLabel().getFont().deriveFont(Font.BOLD);
    private int gap = 5;
    ...
    public JSwitchBox(String trueLabel, String falseLabel) {
        ...
        double trueLength = getFontMetrics(getFont()).getStringBounds(trueLabel, getGraphics()).getWidth();
        double falseLenght = getFontMetrics(getFont()).getStringBounds(falseLabel, getGraphics()).getWidth();
        max = (int)Math.max(trueLength, falseLenght);
        gap = Math.max(5, (int)Math.abs(trueLength - falseLenght));
    ...
    protected void paintComponent(Graphics g) {
        ...
        g2.setColor(Color.BLACK); //black.darker());

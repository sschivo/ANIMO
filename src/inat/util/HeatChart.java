...
public class HeatChart {
	...
	public HeatChart(double[][] zValues, double low, double high) {
		...
		this.cellSizeWidth = this.cellSizeHeight = 20;
		this.margin = 20;
	...
	public void setCellSize(Dimension cellSize) {
		this.cellSizeWidth = cellSize.width;
		this.cellSizeHeight = cellSize.height;
	}
	...
	public Dimension getCellSize() {
		return new Dimension((int)Math.round(this.cellSizeWidth), (int)Math.round(this.cellSizeHeight));
	}
	...
	private void measureComponents() {
		...
		int heatMapWidth = (int)Math.round(zValues[0].length * cellSizeWidth);
		int heatMapHeight = (int)Math.round(zValues.length * cellSizeHeight);
	...
	public void setChartSize(Dimension newDim) {
		measureComponents();
		int yValuesHorizontalSize = 0;
		if (yValuesHorizontal) {
			yValuesHorizontalSize = yAxisValuesWidthMax;
		...
		chartSize = newDim;
		int heatMapWidth = newDim.width - axisThickness - yValuesHorizontalSize - yAxisLabelSize.height - (2 * margin),
			heatMapHeight = newDim.height - axisThickness - titleSize.height - xValuesVerticalSize - xAxisLabelSize.height - (2 * margin);
		cellSizeWidth = 1.0 * heatMapWidth / zValues[0].length;
		cellSizeHeight = 1.0 * heatMapHeight / zValues.length;
	}
	...
	private void drawHeatMap(Graphics2D chartGraphics, double[][] data) {
		...
		for (int x=0; x<data.length; x++) {
			for (int y=0; y<data[x].length; y++) {
				...
				int cellX = (int)Math.round(x * cellSizeHeight)-1; //-1 here and +2 below compensate rounding errors
				int cellY = (int)Math.round(y * cellSizeWidth)-1;
				
				heatMapGraphics.fillRect(cellY, cellX, (int)Math.round(cellSizeWidth)+2, (int)Math.round(cellSizeHeight)+2);
			}
		}
	...
	private void drawXValues(Graphics2D chartGraphics) {
		...
		for (int i=0; i<xValues.length; i++) {
			...
			if (xValuesHorizontal) {
				...
				int valueXPos = (int)Math.round((i * cellSizeWidth) + ((cellSizeWidth / 2) - (valueWidth / 2)));
				valueXPos += heatMapTL.x;
				int valueYPos = heatMapBR.y + metrics.getAscent() + 1;
				
				chartGraphics.drawString(xValueStr, valueXPos, valueYPos);
			} else {
				int valueXPos = (int)Math.round(heatMapTL.x + (i * cellSizeWidth) + ((cellSizeWidth / 2) + (xAxisValuesHeight / 2)));
	...
	private void drawYValues(Graphics2D chartGraphics) {
		...
		for (int i=0; i<yValues.length; i++) {
			...
			if (yValuesHorizontal) {
				...
				int valueYPos = (int)Math.round(heatMapTL.y + (i * cellSizeHeight) + (cellSizeHeight/2) + (yAxisValuesAscent/2));
				
				chartGraphics.drawString(yValueStr, valueXPos, valueYPos);
			} else {
				int valueXPos = margin + yAxisLabelSize.height + yAxisValuesAscent;
				int valueYPos = (int)Math.round(heatMapTL.y + (i * cellSizeHeight) + (cellSizeHeight/2) + (valueWidth/2));
	...
	private Color getCellColour(double data, double min, double max) {
		...
		return getCellColor(percentPosition);
